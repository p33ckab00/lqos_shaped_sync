...
from engine.rust_core import (
    ...
    rust_build_rust_backend_service_runtime_handoff_contract,
    ...
    rust_build_rust_backend_production_enablement_contract,
...
@app.route("/api/rust-core/rust-backend-service-runtime-handoff-contract", methods=["GET", "POST"])
@login_required
def api_rust_core_rust_backend_service_runtime_handoff_contract():
    cfg = load_config(CONFIG_PATH)
    if request.method == "POST":
        payload = request.get_json(silent=True) or {}
    else:
        payload = {
            ...
            "confirmation": request.args.get("confirmation") or "",
            "shadow_age_seconds": int(request.args.get("shadow_age_seconds") or 0),
            "api_route_parity_ready": str(request.args.get("api_route_parity_ready") or "").lower() in {"1", "true", "yes", "on"},
            "api_route_parity_score": float(request.args.get("api_route_parity_score") or 0),
            "webui_ux_unchanged": str(request.args.get("webui_ux_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "static_assets_compat_ready": str(request.args.get("static_assets_compat_ready") or "").lower() in {"1", "true", "yes", "on"},
            "webui_static_asset_paths_unchanged": str(request.args.get("webui_static_asset_paths_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "static_asset_compat_error_count": int(request.args.get("static_asset_compat_error_count") or 0),
            "rust_service_supervision_shadow_ready": str(request.args.get("rust_service_supervision_shadow_ready") or "").lower() in {"1", "true", "yes", "on"},
            "rust_daemon_socket_shadow_ready": str(request.args.get("rust_daemon_socket_shadow_ready") or "").lower() in {"1", "true", "yes", "on"},
            "rust_service_healthcheck_shadow_ready": str(request.args.get("rust_service_healthcheck_shadow_ready") or "").lower() in {"1", "true", "yes", "on"},
            "rust_service_supervision_error_count": int(request.args.get("rust_service_supervision_error_count") or 0),
            "rust_api_shadow_ready": str(request.args.get("rust_api_shadow_ready") or "").lower() in {"1", "true", "yes", "on"},
            "rust_api_response_parity_ready": str(request.args.get("rust_api_response_parity_ready") or "").lower() in {"1", "true", "yes", "on"},
            "rust_api_shadow_error_count": int(request.args.get("rust_api_shadow_error_count") or 0),
            "rust_apply_journal_rollback_authority_handoff_confirmation": request.args.get("rust_apply_journal_rollback_authority_handoff_confirmation") or "CONFIRM_RUST_APPLY_JOURNAL_ROLLBACK_AUTHORITY_HANDOFF_CONTRACT",
        }
    return jsonify(rust_build_rust_backend_service_runtime_handoff_contract(cfg, payload))


@app.route("/api/rust-core/full-rust-backend-production-readiness-contract", methods=["GET", "POST"])
@login_required
def api_rust_core_full_rust_backend_production_readiness_contract():
    cfg = load_config(CONFIG_PATH)
    if request.method == "POST":
        payload = request.get_json(silent=True) or {}
    else:
        payload = {
            ...
            "webui_ux_unchanged": str(request.args.get("webui_ux_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "webui_static_asset_paths_unchanged": str(request.args.get("webui_static_asset_paths_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "operator_final_review_ack": str(request.args.get("operator_final_review_ack") or "").lower() in {"1", "true", "yes", "on"},
            "operator_ack": request.args.get("operator_ack") or "",
            "rust_backend_service_runtime_handoff_confirmation": request.args.get("rust_backend_service_runtime_handoff_confirmation") or "CONFIRM_RUST_BACKEND_SERVICE_RUNTIME_HANDOFF_CONTRACT",
            "rust_apply_journal_rollback_authority_handoff_confirmation": request.args.get("rust_apply_journal_rollback_authority_handoff_confirmation") or "CONFIRM_RUST_APPLY_JOURNAL_ROLLBACK_AUTHORITY_HANDOFF_CONTRACT",
        }
    return jsonify(rust_build_full_rust_backend_production_readiness_contract(cfg, payload))


@app.route("/api/rust-core/full-rust-backend-cutover-plan", methods=["GET", "POST"])
@login_required
def api_rust_core_full_rust_backend_cutover_plan():
    cfg = load_config(CONFIG_PATH)
    if request.method == "POST":
        payload = request.get_json(silent=True) or {}
    else:
        payload = {
            ...
            "webui_ux_unchanged": str(request.args.get("webui_ux_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "webui_static_asset_paths_unchanged": str(request.args.get("webui_static_asset_paths_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "operator_cutover_approval_ack": str(request.args.get("operator_cutover_approval_ack") or "").lower() in {"1", "true", "yes", "on"},
            "operator_ack": request.args.get("operator_ack") or "",
            "rollback_path": request.args.get("rollback_path") or "python_backend_reenable_and_flask_route_restore",
    ...
    return jsonify(rust_build_full_rust_backend_cutover_plan(cfg, payload))


@app.route("/api/rust-core/full-rust-backend-cutover-execution-contract", methods=["GET", "POST"])
@login_required
def api_rust_core_full_rust_backend_cutover_execution_contract():
    cfg = load_config(CONFIG_PATH)
    if request.method == "POST":
        payload = request.get_json(silent=True) or {}
    else:
        payload = {
            ...
            "webui_ux_unchanged": str(request.args.get("webui_ux_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "webui_static_asset_paths_unchanged": str(request.args.get("webui_static_asset_paths_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "operator_cutover_execution_ack": str(request.args.get("operator_cutover_execution_ack") or "").lower() in {"1", "true", "yes", "on"},
            "operator_ack": request.args.get("operator_ack") or "",
            "rollback_path": request.args.get("rollback_path") or "python_backend_reenable_and_flask_route_restore",
...
@app.route("/api/rust-core/rust-backend-production-enablement-contract", methods=["GET", "POST"])
@login_required
def api_rust_core_rust_backend_production_enablement_contract():
    cfg = load_config(CONFIG_PATH)
    if request.method == "POST":
        payload = request.get_json(silent=True) or {}
    else:
        payload = {
            ...
            "webui_ux_unchanged": str(request.args.get("webui_ux_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "webui_static_asset_paths_unchanged": str(request.args.get("webui_static_asset_paths_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "operator_rust_backend_enablement_ack": str(request.args.get("operator_rust_backend_enablement_ack") or "").lower() in {"1", "true", "yes", "on"},
            "operator_ack": request.args.get("operator_ack") or "",
            "rollback_path": request.args.get("rollback_path") or "restore_python_backend_and_flask_routes",
            "python_backend_retirement_plan_confirmation": request.args.get("python_backend_retirement_plan_confirmation") or "CONFIRM_PYTHON_BACKEND_RETIREMENT_PLAN",
            "full_rust_backend_cutover_execution_confirmation": request.args.get("full_rust_backend_cutover_execution_confirmation") or "CONFIRM_FULL_RUST_BACKEND_CUTOVER_EXECUTION_CONTRACT",
        }
    return jsonify(rust_build_rust_backend_production_enablement_contract(cfg, payload))


@app.route("/api/rust-core/python-backend-removal-execution-contract", methods=["GET", "POST"])
@login_required
def api_rust_core_python_backend_removal_execution_contract():
    cfg = load_config(CONFIG_PATH)
    if request.method == "POST":
        payload = request.get_json(silent=True) or {}
    else:
        payload = {
            ...
            "webui_ux_unchanged": str(request.args.get("webui_ux_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "webui_static_asset_paths_unchanged": str(request.args.get("webui_static_asset_paths_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "operator_python_backend_removal_execution_ack": str(request.args.get("operator_python_backend_removal_execution_ack") or "").lower() in {"1", "true", "yes", "on"},
            "operator_ack": request.args.get("operator_ack") or "",
            "rollback_path": request.args.get("rollback_path") or "restore_python_backend_and_flask_routes",
            "rust_backend_production_enablement_confirmation": request.args.get("rust_backend_production_enablement_confirmation") or "CONFIRM_RUST_BACKEND_PRODUCTION_ENABLEMENT_CONTRACT",
            "python_backend_retirement_plan_confirmation": request.args.get("python_backend_retirement_plan_confirmation") or "CONFIRM_PYTHON_BACKEND_RETIREMENT_PLAN",
        }
    return jsonify(rust_build_python_backend_removal_execution_contract(cfg, payload))


@app.route("/api/rust-core/full-rust-backend-removal-rehearsal", methods=["GET", "POST"])
@login_required
def api_rust_core_full_rust_backend_removal_rehearsal():
    cfg = load_config(CONFIG_PATH)
    if request.method == "POST":
        payload = request.get_json(silent=True) or {}
    else:
        payload = {
            ...
            "webui_ux_unchanged": str(request.args.get("webui_ux_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "webui_static_asset_paths_unchanged": str(request.args.get("webui_static_asset_paths_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "operator_full_rust_backend_removal_rehearsal_ack": str(request.args.get("operator_full_rust_backend_removal_rehearsal_ack") or "").lower() in {"1", "true", "yes", "on"},
            "operator_ack": request.args.get("operator_ack") or "",
            "rollback_path": request.args.get("rollback_path") or "restore_python_backend_and_flask_routes",
            "python_backend_removal_execution_confirmation": request.args.get("python_backend_removal_execution_confirmation") or "CONFIRM_PYTHON_BACKEND_REMOVAL_EXECUTION_CONTRACT",
            "rust_backend_production_enablement_confirmation": request.args.get("rust_backend_production_enablement_confirmation") or "CONFIRM_RUST_BACKEND_PRODUCTION_ENABLEMENT_CONTRACT",
        }
    return jsonify(rust_build_full_rust_backend_removal_rehearsal(cfg, payload))


@app.route("/api/rust-core/full-rust-backend-production-cutover", methods=["GET", "POST"])
@login_required
def api_rust_core_full_rust_backend_production_cutover():
    cfg = load_config(CONFIG_PATH)
    if request.method == "POST":
        payload = request.get_json(silent=True) or {}
    else:
        payload = {
            "mode": request.args.get("mode") or "cutover",
            "confirmation": request.args.get("confirmation") or "",
            "shadow_age_seconds": int(request.args.get("shadow_age_seconds") or 0),
            "webui_ux_unchanged": str(request.args.get("webui_ux_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "webui_static_asset_paths_unchanged": str(request.args.get("webui_static_asset_paths_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "webui_static_assets_preserved": str(request.args.get("webui_static_assets_preserved") or "").lower() in {"1", "true", "yes", "on"},
            "operator_full_rust_backend_production_cutover_ack": str(request.args.get("operator_full_rust_backend_production_cutover_ack") or "").lower() in {"1", "true", "yes", "on"},
            "rollback_path": request.args.get("rollback_path") or "restore_python_backend_and_flask_routes",
            ...
            "python_fallback_backup_ready": str(request.args.get("python_fallback_backup_ready") or "").lower() in {"1", "true", "yes", "on"},
...
@app.route("/api/rust-core/full-rust-backend-post-retirement-verifier", methods=["GET", "POST"])
@login_required
def api_rust_core_full_rust_backend_post_retirement_verifier():
    cfg = load_config(CONFIG_PATH)
    if request.method == "POST":
        payload = request.get_json(silent=True) or {}
    else:
        payload = {
            ...
            "python_api_routes_unregistered": str(request.args.get("python_api_routes_unregistered") or "").lower() in {"1", "true", "yes", "on"},
            "python_backend_files_preserved_for_rollback": str(request.args.get("python_backend_files_preserved_for_rollback") or "").lower() in {"1", "true", "yes", "on"},
            "python_backend_rollback_package_ready": str(request.args.get("python_backend_rollback_package_ready") or "").lower() in {"1", "true", "yes", "on"},
            "rollback_path": request.args.get("rollback_path") or "restore_python_backend_and_flask_routes",
            "server_cargo_tests_passed": str(request.args.get("server_cargo_tests_passed") or "").lower() in {"1", "true", "yes", "on"},
            "self_test_passed": str(request.args.get("self_test_passed") or "").lower() in {"1", "true", "yes", "on"},
            ...
            "operator_full_rust_backend_post_retirement_ack": str(request.args.get("operator_full_rust_backend_post_retirement_ack") or "").lower() in {"1", "true", "yes", "on"},
            "full_rust_backend_production_verifier_confirmation": request.args.get("full_rust_backend_production_verifier_confirmation") or "CONFIRM_FULL_RUST_BACKEND_PRODUCTION_VERIFIER",
        }
    return jsonify(rust_build_full_rust_backend_post_retirement_verifier(cfg, payload))


@app.route("/api/rust-core/full-rust-backend-steady-state-guard", methods=["GET", "POST"])
@login_required
def api_rust_core_full_rust_backend_steady_state_guard():
    cfg = load_config(CONFIG_PATH)
    if request.method == "POST":
        payload = request.get_json(silent=True) or {}
    else:
        payload = {
            "mode": request.args.get("mode") or "guard",
            "confirmation": request.args.get("confirmation") or "",
            "shadow_age_seconds": int(request.args.get("shadow_age_seconds") or 0),
            "webui_ux_unchanged": str(request.args.get("webui_ux_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            "webui_static_asset_paths_unchanged": str(request.args.get("webui_static_asset_paths_unchanged") or "").lower() in {"1", "true", "yes", "on"},
            ...
            "operator_full_rust_backend_steady_state_ack": str(request.args.get("operator_full_rust_backend_steady_state_ack") or "").lower() in {"1", "true", "yes", "on"},
