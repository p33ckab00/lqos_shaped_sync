...
## Rust Core v6.2 commit suggestion

```bash
git add .
git commit -m "rust(core): add full Rust backend cutover execution contract" \
...
## Rust Core v7.3.3 hotfix commit

```bash
git add .
git commit -m "rust(core): fix steady-state guard webui ux self-test gate" \
...
## v7.5.3 cleanup execution commit

```bash
git add .
git commit -m "ops(cleanup): add stale codebase cleanup execution guard" \
